...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_KANGDROID
static
#endif
struct cpufreq_governor cpufreq_gov_kangdroid = {
	.name = "kangdroid",
	.governor = cpufreq_governor_kangdroid,
...
static int cpufreq_governor_kangdroid(struct cpufreq_policy *policy,
		unsigned int event)
{
	int rc;
	unsigned int j;
...
static int __init cpufreq_kangdroid_init(void)
{
	unsigned int i;
	struct cpufreq_interactive_cpuinfo *pcpu;
	...
	return cpufreq_register_governor(&cpufreq_gov_kangdroid);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_KANGDROID
fs_initcall(cpufreq_kangdroid_init);
#else
module_init(cpufreq_kangdroid_init);
#endif

static void __exit cpufreq_interactive_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_kangdroid);
...
MODULE_DESCRIPTION("'cpufreq_kangdroid' - A cpufreq governor for "
	"Latency sensitive workloads based on Google, faux, hellsactive, CAF and some patches from franciscofranco");
MODULE_LICENSE("GPL");
